...
const directionArrowPlugin = {
  id: 'directionArrow',
  afterDatasetsDraw(chart, args, pluginOptions) {
    const angleStd = pluginOptions && typeof pluginOptions.angle === 'number' ? pluginOptions.angle : null;
    if (angleStd === null || !isFinite(angleStd)) return;
    const scale = chart.scales.r;
    if (!scale) return;

    const cx = scale.xCenter;
    const cy = scale.yCenter;
    const r = Math.max(18, scale.drawingArea * 0.72);

    // Display convention: 0° = right, counter-clockwise positive.
    // Canvas convention: +x right, +y down, so use negative angle.
    const rad = -angleStd * Math.PI / 180;
    ...
    ctx.beginPath();
    ctx.moveTo(x2, y2);
    ctx.lineTo(x2 - headLen * Math.cos(rad - headAng), y2 - headLen * Math.sin(rad - headAng));
    ctx.moveTo(x2, y2);
    ctx.lineTo(x2 - headLen * Math.cos(rad + headAng), y2 - headLen * Math.sin(rad + headAng));
    ctx.lineWidth = 5;
    ctx.strokeStyle = '#ef4444';
 ...
 * Initialize the angle time series chart
  ...
  function initCharts() {
    if (typeof Chart !== 'undefined') {
      Chart.register(directionArrowPlugin);
    }
    initPolarChart();
    initAngleChart();
    initDistanceChart();
